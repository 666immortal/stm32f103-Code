...
static  void W25QxxEraseChip(void);           //整片擦除
static  void W25QxxEraseSector(u32 Dst_Addr); //扇区擦除

static  void W25QxxPowerDown(void);           //进入掉电模式
static  void W25QxxWakeUp(void);              //唤醒
...
{  
  ...
  SPIReadWriteByte(W25X_READ_STS_REG);      //发送读取状态寄存器命令    
...
{   
  CLR_W25Qxx_CS();                          //选中W25Qxx的片选
  SPIReadWriteByte(W25X_WRITE_STS_REG);     //发送写取状态寄存器命令    
  SPIReadWriteByte(sr);                     //写入一个字节  
  SET_W25Qxx_CS();                          //取消W25Qxx的片选
} 
...
{
  CLR_W25Qxx_CS();                            //选中W25Qxx的片选
  SPIReadWriteByte(W25X_WRITE_ENABLE);        //发送写使能指令
  SET_W25Qxx_CS();                            //取消W25Qxx的片选
} 
...
{  
  CLR_W25Qxx_CS();                            //选中W25Qxx的片选
  SPIReadWriteByte(W25X_WRITE_DISABLE);       //发送写禁止指令    
  SET_W25Qxx_CS();                            //取消W25Qxx的片选
}
...
static  void W25QxxWritePage(u8* pBuffer, u32 WriteAddr, u16 NumByteToWrite)
{
  ...
  SPIReadWriteByte(W25X_PAGE_PROGRAM);        //发送写页命令   
  SPIReadWriteByte((u8)((WriteAddr) >> 16));  //发送24bit地址    
  SPIReadWriteByte((u8)((WriteAddr) >> 8));   
  SPIReadWriteByte((u8)WriteAddr);   
  for(i = 0; i < NumByteToWrite; i++)
  {
    SPIReadWriteByte(pBuffer[i]);             //循环写数
  }
...
{                                             
  ...
  W25QxxWaitBusy();                     //等待空闲   
  CLR_W25Qxx_CS();                      //选中W25Qxx的片选
  SPIReadWriteByte(W25X_CHIP_ERASE);    //发送片擦除命令  
  SET_W25Qxx_CS();                      //取消W25Qxx的片选
...
{   
  ...
  W25QxxWaitBusy();                           //等待空闲
  CLR_W25Qxx_CS();                            //选中W25Qxx的片选
  SPIReadWriteByte(W25X_SECTOR_ERASE);        //发送扇区擦除指令 
  SPIReadWriteByte((u8)((Dst_Addr) >> 16));   //发送24bit地址    
...
{ 
  CLR_W25Qxx_CS();                            //选中W25Qxx的片选
  SPIReadWriteByte(W25X_POWER_DOWN);          //发送掉电模式命令  
  SET_W25Qxx_CS();                            //取消W25Qxx的片选
...
{  
  CLR_W25Qxx_CS();                              //选中W25Qxx的片选
  SPIReadWriteByte(W25X_RELEASE_POWER_DOWN);    //发送释放掉电模式命令
  SET_W25Qxx_CS();                              //取消W25Qxx的片选
...
void InitW25Qxx(void)
{
  ...
  RCC_APB2PeriphClockCmd(USER_DEFINE_SPI_CS_GPIO_CLK, ENABLE);   //使能W25Qxx片选的时钟

  GPIO_InitStructure.GPIO_Pin   = USER_DEFINE_SPI_CS_GPIO_PIN;   //设置25Qxx片选的引脚    
  GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_Out_PP;              //推挽输出模式
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;              //设置I/O口速率为50MHz
  GPIO_Init(USER_DEFINE_SPI_CS_GPIO_PORT, &GPIO_InitStructure);  //根据参数初始化25Qxx片选的GPIO端口

  SET_W25Qxx_CS();                                               //取消W25Qxx的片选
  InitSPI();                                                     //初始化SPI
  SPISetSpeed(SPI_BaudRatePrescaler_2);                          //设置为18M时钟,高速模式
}
...
u16 W25QxxReadID(void)
{
  ...
  SPIReadWriteByte(0x90);                     //发送读取ID命令    
  SPIReadWriteByte(0x00);         
  SPIReadWriteByte(0x00);         
  SPIReadWriteByte(0x00);         
  ID |= SPIReadWriteByte(0xFF) << 8;          //将第一次读取到的字节左移8位并赋给ID高八位
  ID |= SPIReadWriteByte(0xFF);               //将第二次读取到的字节赋给ID低八位
  SET_W25Qxx_CS();                            //取消W25Qxx的片选
...
{ 
  ...
  SPIReadWriteByte(W25X_READ_DATA);           //发送读取命令   
  SPIReadWriteByte((u8)((ReadAddr) >> 16));   //发送24bit地址    
  SPIReadWriteByte((u8)((ReadAddr) >> 8));   
  SPIReadWriteByte((u8)ReadAddr);   
  
  for(i = 0; i < NumByteToRead; i++)
  { 
    pBuffer[i] = SPIReadWriteByte(0XFF);      //循环读数  
  }
